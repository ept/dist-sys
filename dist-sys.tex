...
\begin{frame}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \node [rectangle,fill=red!10,draw] (server) at (8,4) {server www.cit.tum.de};
            \draw (client) -- (0,0);
            \draw (server) -- (8,0);
            \draw<2-> [bigarrow] (0,3) -- node [above,sloped] {GET /en/cit/home/} (8,2);
...
In a URL such as \url{https://www.cit.tum.de/en/cit/home/}, the part between the \verb|//| and the following \verb|/| is the hostname of the server to which the client is going to send the request (\texttt{www.cit.tum.de}), and the rest (\texttt{/en/cit/home/}) is the path that the client asks for in its request message.
...
We don't particularly care whether those nodes are servers, desktop computers, phones, sensors, or any other kind of computing device.
Moreover, we don't care how messages are physically represented or encoded~-- the network protocols, informally known as the \emph{bytes on the wire}~-- because the basic principle of sending and receiving messages remains the same, even as particular networking technologies come and go.
...
Moreover, fault tolerance has operational benefits: a single computer might need to be rebooted from time to time (e.g.\ to install software updates), but if you are using multiple nodes, then one node can continue serving users while another node is rebooting.
...
The primary goal of most practical work is to ensure the system provides consistently good performance, especially as the system grows to handle larger volumes of requests and data (\emph{scalability}).
...
\begin{frame}
    ...
    $\Rightarrow$ works fine\dots\\
    $\Rightarrow$ but one day it fails catastrophically due to some weird edge case, and corrupts all your data \pause\\[1em]
...
% TODO: add crying emoji to "corrups all your data", exclamation mark to "hard to get right"
...
The next few lectures by Dr.\ Martin Kleppmann will introduce the theoretical perspective on distributed systems, after which Prof.\ Pramod Bhatotia will switch back to the practical perspective.
...
\begin{frame}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw (client) -- (0,0);
            \draw (server) -- (8,0);
            \draw<2-> [bigarrow] (0,3) -- node [above,sloped] {charge {\euro}3.99 to credit card 1234\dots} (8,2);
...
\begin{frame}
    ...
    Message-passing distributed systems: \uncover<2->{$\longleftarrow$ \textbf{this course}}
    \begin{center}
        \begin{tikzpicture}
...
Shared-memory concurrency is a common model when you have several threads of execution running on the same computer.
...
\begin{frame}
    ...
    \begin{itemize}
        \item Multiple ``nodes'' (computers)
            \begin{itemize}
                \item node = server, desktop computer, phone, car, robot, \dots
            \end{itemize}
        \item communicating via a network
            \begin{itemize}
                \item by sending each other messages
...
This course focuses on the message-passing model, which is the basis of most practical distributed systems.
...
In the theory part of this course we don't distinguish them: a node can be any type of communicating computing device.
...
\begin{frame}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw [doublebigarrow] (army1) -- node [below] {messenger doves} (army2);
        \end{tikzpicture}\\[1em]\pause
        \rowcolors[]{2}{}{blue!10}
        \renewcommand{\arraystretch}{1.3}
        \begin{tabular}{c|c|c}
            \hline
            Romeo & Juliet & outcome \\\hline
            does not attend & does not attend & nothing happens \\
            attends & does not attend & Romeo gets desperate \\
            does not attend & attends & Juliet gets desperate \\
            attends & attends & happy ever after \\\hline
        \end{tabular}\\[1em]
...
What protocol should the two generals (or Romeo and Juliet) use to agree on a plan?
...
\begin{frame}
    ...
    \end{enumerate}
    The fact that one of the actions can be undone makes the problem solveable.\\[1em]
...
\begin{frame}
    ...
    \frametitle{ABD: Linearizable quorum reads/writes}
    \begin{tikzpicture}
        \node [rectangle,fill=red!10,draw] (node1) at (0,5.9) {client 1};
        \node [rectangle,fill=red!10,draw] (node2) at (2,5.9) {\storageSymbol}; \node [yshift=-0.2cm] at (node2) {$A$};
        \node [rectangle,fill=red!10,draw] (node3) at (4,5.9) {\storageSymbol}; \node [yshift=-0.2cm] at (node3) {$B$};
