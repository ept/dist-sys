...
% A less militaristic analogy: The Romeo-and-Juliet problem.
% R&J want to secretly meet in the forest and need to agree on a date. But if
% only one of them arrives, the other becomes desperate and a tragedy occurs.
% Unfortunately, their way of communication via doves is not very reliable.
% They get distracted or lost or shot...
% https://twitter.com/anne_biene/status/1386975750671187970
        ...
        \begin{minipage}{11cm}
            ...
            \begin{algorithmic}
                \On{receiving $(\mathsf{LogRequest}, \mathit{leaderId}, \mathit{term}, \mathit{logLength}, \mathit{logTerm},$\\\hspace{58pt}$\mathit{leaderCommit}, \mathit{entries})$ at node $\mathit{nodeId}$}
                    ...
                    \EndIf
                    \If{$\mathit{term} = \mathit{currentTerm} \wedge \mathit{currentRole} = \mathsf{candidate}$}
                        \State $\mathit{currentRole} := \mathsf{follower};\; \mathit{currentLeader} := \mathit{leaderId}$
                    \EndIf
...
This algorithm is shown on the next two slides.
