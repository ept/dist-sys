...
\begin{frame}
    \label{s:raft8}
    \frametitle{Raft (8): leader committing log entries}
    \footnotesize
    \makebox[\linewidth][c]{%
        \begin{minipage}{12cm}
            \raggedright
                ...
                \On{receiving $(\mathsf{LogResponse}, \mathit{follower}, \mathit{term}, \mathit{ack}, \mathit{success})$ at $\mathit{nodeId}$}
                    \If{$\mathit{term} = \mathit{currentTerm}$}
                        \If{$\mathit{success} = \mathsf{true}$}
                            \State $\mathit{sentLength}[\mathit{follower}] := \mathit{ack};\; \mathit{ackedLength}[\mathit{follower}] := \mathit{ack}$
                            ...
                            \If{$\mathit{ready} \ne \{\} \;\wedge\; \max(\mathit{ready}) > \mathit{commitLength} \;\wedge$\\\hspace{75pt}$\mathit{log}[\max(\mathit{ready}) - 1].\mathsf{term} = \mathit{currentTerm}$}
...
Raft \citep{Ongaro:2014} and Paxos \citep{Lamport:1998}.
