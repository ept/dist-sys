...
At the moment where we commit a log entry, we also deliver its $\mathsf{msg}$ to the application.
...
When the node suspects a leader failure, starts a leader election as follows: it increments $\mathit{currentTerm}$, it sets its own role to $\mathsf{candidate}$, and it votes for itself by setting $\mathit{votedFor}$ and $\mathit{votesReceived}$ to its own node ID.
...
The pseudocode uses zero-based indexing, so $\mathit{log}[0]$ is the first log entry and $\mathit{log}[\mathit{log}.\mathsf{length}-1]$ is the last.
...
\begin{frame}
    ...
    \begin{algorithmic}
        ...
        \On{node $\mathit{nodeId}$ suspects leader has failed, or on election timeout}
            ...
            \State $\mathit{currentRole} := \mathsf{candidate};\; \mathit{votedFor} := \mathit{nodeId}$
...
As its first action as a leader, it updates the $\mathit{sentLength}$ and $\mathit{ackedLength}$ variables, which will be explained later, and then calls the \textsc{ReplicateLog} function (defined on \autoref{l:raft5}) for each follower.
...
\autoref{l:raft4} shows how new entries are added to the log.
