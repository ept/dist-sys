...
\begin{frame}
    ...
    \begin{tikzpicture}[every node/.style={font=\footnotesize,align=center}]
        ...
        \draw<2-> [bigarrow] (app1)       -- node [right,orange] {\textbf{broadcast}} (mid1);
        \draw<3-> [bigarrow] (mid1.south) -- node [right,red]    {\textbf{send}}      (0,0.25);
        \draw<3-> [bigarrow] (6,0.25)     -- node [right,violet] {\textbf{receive}}   (mid2.south);
...
\begin{frame}
    ...
    \frametitle{Three forms of reliable broadcast}
    \textbf{FIFO broadcast}:\\
    If $m_1$ and $m_2$ are sent by the same node, and $\mathsf{send}(m_1) \rightarrow \mathsf{send}(m_2)$, then $m_1$ must be delivered before $m_2$\\[2em]\pause
    \textbf{Causal order broadcast}:\\
    If $\mathsf{send}(m_1) \rightarrow \mathsf{send}(m_2)$ then $m_1$ must be delivered before $m_2$\\[2em]\pause
...
This may seem unnecessary~-- after all, a node knows what messages it has itself broadcast!~-- but setting up the protocol this way makes it easier to formalise.
...
\begin{frame}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            % broadcast m2
            \node [inner sep=3pt] (send2) at (4,2.2) {};
            \path<2-> [->,very thick] (send2) edge [rightloop] node {$m_2$} (send2);
            \draw<2-> [bigarrow] (4,2.4) -- node [above,near start] {$m_2$} (0,0.8);
            \draw<2-> [bigarrow] (4,2.3) -- (8,0.6);
            % broadcast m3
            \node [inner sep=2pt] (send3) at (0,2.0) {};
            \path<3-> [->,very thick] (send3) edge [leftloop] node {$m_3$} (send3);
            \draw<3-> [bigarrow] (0,2.0) -- node [above,pos=0.15] {$m_3$} (8,1.2);
    ...
    \uncover<5->{Here: $\mathsf{send}(m_1) \rightarrow \mathsf{send}(m_2)$ and $\mathsf{send}(m_1) \rightarrow \mathsf{send}(m_3)$\\
    so valid orders are: $(m_1, m_2, m_3)$ or $(m_1, m_3, m_2)$}
\end{frame}
