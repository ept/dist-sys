...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item \textbf{logical clocks}: count events, e.g.\ messages sent
    \end{itemize}\pause
...
\begin{frame}
    ...
    \begin{columns}
        \begin{column}{0.7\textwidth}
            ...
            \textbf{International Atomic Time} (TAI): 1 day is $24 \times 60 \times 60 \times \text{9,192,631,770}$ periods of caesium-133's resonant frequency\\[1em]
            \textbf{Problem}: speed of Earth's rotation is not constant\\[1em]
            \textbf{Compromise}: UTC is TAI with corrections to account for Earth rotation\\[1em]
            \textbf{Time zones} and \textbf{daylight savings time} are offsets to UTC
        \end{column}
...
Two common representations of timestamps are used: Unix time and ISO~8601, as shown on \autoref{l:time-representation}.
...
\begin{frame}
    ...
    \begin{itemize}
        \item \textbf{Unix time}: number of seconds since 1 January 1970 00:00:00 UTC, \emph{not counting leap seconds} (!)
...
(The best solution is probably to use a \emph{monotonic clock} instead, which we introduce in Section TODO.)
...
\supervision{
    ...
    Time measurements will be slightly wrong during the smearing period.
    For example, if the extra second is spread over 24 hours (12 hours before and 12 hours after the leap second), this amounts to an error of about 12~ppm.
    ...
    For example, this could arise if two nodes attempt to measure the network latency between them by sending one node's current timestamp as a message to the other node, and calculating the difference of timestamps when the message is received.
    ...
    Our department's Markus Kuhn tried to get smearing standardised in 2006: \url{https://www.cl.cam.ac.uk/~mgk25/time/utc-sls/}
}

\subsection{Physical clock synchronisation}\label{sec:clock-sync}

\begin{frame}
    \label{s:clock-sync}
    \frametitle{Clock synchronisation}
    Computers track physical time/UTC with a quartz clock (with battery, continues running when power is off).\\[1em]
    Due to \textbf{clock drift}, local clock gradually becomes more and more wrong\\[1em]
    \textbf{Clock skew}: difference between two clocks at a point in time\\[1em]
    \textbf{Solution}: Periodically get the current time from a server using NTP or PTP
\end{frame}
\inlineslide{s:clock-sync}\label{l:clock-sync}

% An atomic clock can tell you exactly when 20 seconds have elapsed, but (by itself) it cannot tell you whether it's Monday or Tuesday!

% Google Spanner assumes maximum drift of 200 ppm
