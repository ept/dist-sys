            ...
            \begin{algorithmic}
                \On{receiving $(\mathsf{LogRequest}, \mathit{leaderId}, \mathit{term}, \mathit{logLength}, \mathit{logTerm},$\\\hspace{58pt}$\mathit{leaderCommit}, \mathit{entries})$ at node $\mathit{nodeId}$}
                    ...
                    \If{$\mathit{term} = \mathit{currentTerm} \wedge \mathit{logOk}$}
                        % It might be that the local node is a candidate in the same term as the received LogRequest;
                        % in this case we must transition to follower state. Strictly speaking, the next line should be
                        % conditional only on term=currentTerm and not on logOk, but I'm putting it in the same "if"
                        % statement to make it fit on the slide. I don't think the extra "and logOk" does any harm.
