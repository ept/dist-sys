...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item If some nodes are faulty, the others are still accessible\pause
        \item A node that has a copy is called a \textbf{replica}\pause
        \item Easy if the data doesn't change: just copy it
        \item We will focus on data changes\\[2em]\pause
    \end{itemize}
    Compare to \textbf{RAID} (Redundant Array of Independent Disks): replication in a single computer
    \begin{itemize}
        \item RAID has single controller; in distributed system, each node acts independently
        \item Replicas can be distributed around the world
    \end{itemize}
\end{frame}
\inlineslide{s:replication-intro}\label{l:replication-intro}

\subsection{Manipulating state}
