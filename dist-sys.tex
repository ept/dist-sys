...
In the example on \autoref{l:lamport-example}, the first event on node $A$ and the first event on node $C$ both have a timestamp of 1.
...
However, if $a \parallel b$ we could have either $a \prec b$ or $b \prec a$, so the order of the two elements is determined arbitrarily by the algorithm.
...
\begin{frame}
    ...
    \end{itemize}
    Intuition: $V(a) \le V(b)$ if $\{a \text{ and its causal dependencies}\} \subseteq \{b \text{ and its causal dependencies}\}$\\[1em]
