...
\newcommand{\coursepath}{/teaching/2526/ConcDisSys/}
...
\newcommand{\networking}{https://www.cl.cam.ac.uk/teaching/2526/CompNet/}
\newcommand{\security}{https://www.cl.cam.ac.uk/teaching/2526/CySecurity/}
\newcommand{\multicore}{https://www.cl.cam.ac.uk/teaching/2526/MSP/}
...
\begin{frame}
    ...
    {\scriptsize\url{https://www.theguardian.com/technology/2014/aug/14/google-undersea-fibre-optic-cables-shark-attacks}}\\[1em]
...
\begin{frame}
    ...
    \begin{algorithmic}
        ...
        \On{receiving $(T', m)$ at node $N_i$ via the network}
            \State $T[j] := \mathrm{max}(T[j], T'[j])$ for every $j \in \{0, \dots, n-1\}$
...
% TODO: in the diagram below, show all three possible delivery orders, not just two
...
% TODO: visualise the operation of the above algorithm, and also the following causal broadcast algorithm.
% Can show the visualisation and the pseudocode side-by-side in the lecture using both projectors
...
% TODO: would be good to represent those two algorithms diagramatically rather than in prose
...
% TODO: could drop this slide to save a bit of time -- it's not really necessary
...
\begin{itemize}
    \item As discussed in \autoref{sec:delivery-order}, when a node wants to broadcast a message through a total order broadcast, it cannot immediately deliver that message to itself.
    ...
    \item The fault tolerance of state machine replication depends on the fault tolerance of the underlying total order broadcast protocol.
        A good protocol will be able to tolerate some faults: that is, while one node is down, the remaining nodes can continue broadcasting and delivering messages, and when the faulty nodes recovers it will deliver the backlog of messages that were broadcast while it was down.
        We will discuss this further in \autoref{sec:consensus}.
\end{itemize}
...
In some systems, the coordinator is a part of the client.
Note that the coordinator only handles the process of committing or aborting a transaction, and is not involved in the execution of the rest of the transaction.
...
% TODO: even if the coordinator is made fault-tolerant, there's still the problem that a single faulty participating replica results in all transactions getting aborted.
% Also mention that 2PC offers no concurrency control (so writes may be ordered differently on different replicas) and no detection of deadlocks that span multiple replicas (other than a timeout)?
...
% TODO: visualise this algorithm graphically. or maybe drop this algorithm to save time?
...
The \href{\multicore}{Multicore Semantics and Programming} module in Part~II goes into detail of multicore memory behaviour.
...
The $\mathsf{get}$ operation returns its value to the application only after the client is sure that the most recent value is stored on a quorum of replicas: that is, after a quorum of replicas either responded $\mathsf{ok}$ to the read repair, or replied with the most recent value in the first place.
...
% TODO: add a slide explaining single-writer ABD and the multi-writer generalisation, including logical timestamps construction and visualising the timestamp-reading phase.
...
% TODO: add an exercise to show why ABD can't do CAS individually on each replica (or use the lowest timestamp instead of the greatest), and can't roll back either
...
% TODO: move the following slide elsewhere? it's a bit of a non-sequitur: the previous slide ends on conflict resolution, so it would be good to go straight into that
...
% TODO: move this slide to the end, so that we go straight into conflict resolution
