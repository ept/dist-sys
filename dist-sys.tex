...
\vspace{10pt}\noindent Thank you to Alexandre Fruchaud and Rishabh Jain for reporting mistakes in an earlier version of these notes.
...
\supervision{
    ...
    \begin{algorithmic}
        \On{request to read value for key $k$}
            ...
            \State \textbf{send} $(\mathsf{set}, \mathit{maxt}, k, \mathit{maxv})$ to each replica in $\mathit{replicas} \setminus \{i \mid (i, \mathit{maxt}, \mathit{maxv}) \in \mathit{responses}\}$
        ...
        \On{request to set key $k$ to value $v$}
            ...
            \State \textbf{send} $(\mathsf{set}, t, k, v)$ to each replica in $\mathit{replicas}$
            \State $\mathit{responses} := \{\}$
            \While{$|\mathit{responses}| < \left\lceil \frac{|\mathit{replicas}| + 1}{2} \right\rceil$}
                \State \textbf{receive response} $\mathsf{ok}$ from replica $i$
