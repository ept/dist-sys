...
\supervision{
    ...
    (b)~What are the possible values of the \emph{commitLength} variable at each node?
    (c)~Node A starts a leader election in term 4, while the nodes are in the state above.
...
Let's start with a consistency problem that arises when executing a transaction that reads or writes data on multiple nodes (a \emph{distributed transaction}).
...
\begin{frame}
    ...
    What if the coordinator crashes between phases 1 and 2?
    \begin{itemize}
        \item 
    \end{itemize}
\end{frame}
...
Use a consensus protocol to tolerate coordinator faults \citep{Gray:2006}.
