...
\autoref{l:paxos-commit} shows a fault-tolerant two-phase commit algorithm based on Paxos Commit \citep{Gray:2006}.
...
\begin{frame}
    ...
    \footnotesize
    \begin{algorithmic}
        \On{initialization for transaction $T$}
...
On \autoref{l:read-after-write-2} and the following slides we represent the client's view of a $\mathsf{get}$/$\mathsf{set}$ operation as a rectangle, where the top border represents starting and the bottom border represents finishing.
...
\begin{frame}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw<4-> [messageloss] (0.3,1.4) -- node [above,pos=0.2,sloped] {$\mathsf{read}\; x$} (8,0.6);
...
\begin{frame}
    ...
    \begin{tikzpicture}
        ...
        \node<3-> [op,text width=1.8cm] at (8,3.5) {$\mathsf{get}\; x \rightarrow v_1$};
        ...
        \node<5-> [op,text width=1.8cm] at (10,1.0) {$\mathsf{get}\; x \rightarrow v_0$};
...
\begin{frame}
    ...
    \begin{tikzpicture}
        ...
        \node [op,text width=1.8cm] at (8,3.5) {$\mathsf{get}\; x \rightarrow v_1$};
        \node [op,text width=1.8cm] at (10,1.0) {$\mathsf{get}\; x \rightarrow v_0$};
...
\begin{frame}
    ...
    \begin{tikzpicture}
        ...
        \draw<4-> [bigarrow] (7.7,2.1) -- node [above,pos=0.3,sloped,font=\footnotesize,yshift=-0.1cm] {$(t_1, \mathsf{set}\; x=v_1)$} (4,1.7);
        ...
        \draw<5-> [bigarrow] (4,1.2) -- node [above,sloped,near start,font=\footnotesize] {ok} (7.7,0.8);
        \node<1-> [op,text width=1.8cm] (op3) at (10,1.0) {$\mathsf{get}\; x \rightarrow v_1$};
...
% \item Every operation takes effect \textbf{atomically} sometime after it started and before it finished
% \item All operations behave as if executed on a \textbf{single copy} of the data (even if there are in fact multiple replicas)
% \item If operation A finished before operation B started (in \textbf{real time}), then A must take effect before B

% Communication that happens outside of the computer system
