...
A more refined approach to conflict resolution might merge the update to the time and the title into a calendar event that reflects both updates.
...
\subsection{Conflict-free replicated datatypes (CRDTs)}

\subsection{Google's Spanner}
...
\begin{frame}
    ...
    \uncover<3->{On commit, wait for uncertainty $\delta_i = t_{i,\mathsf{latest}} - t_{i,\mathsf{earliest}}$.}\\[1em]
    \begin{tikzpicture}
        ...
        \node<1-> [rectangle,fill=white,draw,text depth=0.3cm,minimum width=0.7cm] (T1) at (1,3) {$T_1$};
        \node<4-> [rectangle,fill=white,draw,text depth=0.3cm,minimum width=0.7cm] (T2) at (7,1) {$T_2$};
        ...
        \fill<4-> [blue!10] (T2.south west) -- (4,1.0) -- (4,0.0) -- cycle;
        ...
        \draw<4-> [blue] (3.6,1.0) -- node [at end,above right,inner sep=0pt] {$t_{2,\mathsf{earliest}}$} (4.2,1.0);
        \draw<4-> [blue] (3.6,0.0) -- node [at end,below right,inner sep=0pt] {$t_{2,\mathsf{latest}}$} (4.2,0.0);
        \draw<3-> [blue,<->] (4.2,3.0) -- node [right] {$\delta_1$} (4.2,2.0);
        \draw<5-> [blue,<->] (3.8,1.0) -- node [left] {$\delta_2$} (3.8,0.0);
