...
If the $\mathsf{LogRequest}$ message is for the expected term and if $\mathit{logOk}$, then the follower accepts the message and calls the \textsc{AppendEntries} function (defined on \autoref{l:raft7}) to add $\mathit{entries}$ to its own log.
        ...
        \begin{minipage}{11cm}
            ...
            \begin{algorithmic}
                \On{receiving $(\mathsf{LogRequest}, \mathit{leaderId}, \mathit{term}, \mathit{logLength}, \mathit{logTerm},$\\\hspace{58pt}$\mathit{leaderCommit}, \mathit{entries})$ at node $\mathit{nodeId}$}
                    ...
                    \EndIf
                    \State $\mathit{logOk} := (\mathit{log}.\mathsf{length} \ge \mathit{logLength}) \;\wedge$
                    \State \hspace{41.5pt}$(\mathit{logLength} = 0 \vee \mathit{logTerm} = \mathit{log}[\mathit{logLength} - 1].\mathsf{term})$
