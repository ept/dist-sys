...
\subsection{Clock synchronisation and NTP}\label{sec:clock-sync}
...
\begin{frame}
    ...
    Due to \textbf{clock drift}, clock error gradually increases\\[1em]
...
\begin{frame}
    ...
    \end{itemize}
    Many operating system vendors run NTP servers and configure devices to use them by default\\[1em]
...
The estimated skew $\theta$ could therefore be too high or too low by up to $\delta/2$.
...
\begin{frame}
    ...
    \begin{itemize}
        \item Time since a fixed date (e.g.\ 1 January 1970 epoch)
        \item May suddenly move forwards or backwards
        \item Timestamps can be compared across nodes (if synced)
        \item Java: \texttt{System.currentTimeMillis()}
        \item Linux: \texttt{clock\_gettime(CLOCK\_REALTIME)}\\[1em]
    \end{itemize}
    \textbf{Monotonic clock:}
    \begin{itemize}
        \item Time since arbitrary point (e.g.\ when machine booted up)
        \item Always moves forwards at near-constant rate
        \item Good for measuring elapsed time on a single node
        \item Java: \texttt{System.nanoTime()}
        \item Linux: \texttt{clock\_gettime(CLOCK\_MONOTONIC)}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \frametitle{Ordering of messages using timestamps?}
    \begin{center}
        \begin{tikzpicture}
            \node [rectangle,fill=red!10,draw] (user1) at (0,3.7) {user A};
            \node [rectangle,fill=red!10,draw] (user2) at (4,3.7) {user B};
...
This is a \emph{partial order}, which means that for some events $a$ and $b$, it is neither the case that $a$ happened before $b$, nor that $b$ happened before $a$.
...
\begin{frame}
    ...
    \end{center}
    \begin{itemize}
        \item $a \rightarrow b$, $c \rightarrow d$, and $e \rightarrow f$ due to process order
        \item $b \rightarrow c$ and $d \rightarrow f$ due to messages $m_1$ and $m_2$
        \item $a \rightarrow c$, $a \rightarrow d$, $a \rightarrow f$, $b \rightarrow d$, $b \rightarrow f$, and $c \rightarrow f$ due to transitivity
...
% Causality: m1: "The moon is made of cheese" --> m2: "Oh no it isn't"
% m3: "Pigs can fly"
