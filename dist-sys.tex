...
\begin{frame}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw<2-> [doublebigarrow] (left1) -- node [above] {reconcile state} (right1);
            \draw<3-> [bigarrow] (left1) -- (left2);
            \draw<3-> [bigarrow] (right1) -- (right2);
            \node<3-> at (4,2.75) {$t_1 < t_2$};
            \node at (0,1.8) {}; % placeholder
        \end{tikzpicture}
    \end{center}
    \begin{uncoverenv}<4->
        \begin{algorithmic}
            \On{receiving request $(t, \mathit{key}, \mathit{value})$}
                \If{$\mathit{key} \notin \mathit{database}$}
                    \State add $\mathit{key} \mapsto (t, \mathit{value})$ to $\mathit{database}$
                \ElsIf{$\exists t', v'.\; \mathit{database}[\mathit{key}] = (t', v') \wedge t' < t$}
                    \State update $\mathit{key} \mapsto (t, \mathit{value})$ in $\mathit{database}$
                \EndIf
            \EndOn
        \end{algorithmic}
    \end{uncoverenv}
...
% slide showing concurrent updates to the same record. user record, concurrently updating different fields?
% last write wins and detecting concurrently written values.
% dotted version vectors

\supervision{
    Apache Cassandra, a widely-used distributed database, uses a replication approach similar to the one described here.
    However, it uses physical timestamps instead of logical timestamps, as discussed in \url{https://www.datastax.com/blog/2013/09/why-cassandra-doesnt-need-vector-clocks}.
    ...
    The article's claim that ``clock synchronization is nice to have in a Cassandra cluster but not critical'' is completely wrong.
...
% Replication: without broadcast (ABD, quorums); 2-phase commit;
% with broadcast (total order: state machine replication, causal: CRDTs)
