...
This can be done, and we will see some techniques for commutativity in \autoref{sec:consistency}.
...
\begin{frame}
    ...
    \begin{uncoverenv}<5->
        \begin{tikzpicture}
            ...
            \node<7-> [red,text width=6.5cm]  at (7,0.45) {\textbf{cannot elect a different leader} because $C$ knows about first leader};
            \node (A) at (0,2) {\storageSymbol}; \node [yshift=-0.2cm] at (A) {$A$};
            \node (B) at (2,2) {\storageSymbol}; \node [yshift=-0.2cm] at (B) {$B$};
            \node (C) at (4,2) {\storageSymbol}; \node [yshift=-0.2cm] at (C) {$C$};
            \node (D) at (6,2) {\storageSymbol}; \node [yshift=-0.2cm] at (D) {$D$};
...
\begin{frame}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw (-1,3) edge [transition] node [label,right,near start,text width=2cm] {starts up or recovers from crash} (follower);
            \draw (follower) edge [transition,out=30,in=150] node [label,above,text width=2.5cm] {suspects leader failure} (candidate);
            \draw (candidate) edge [transition] node [label,below,text width=2.5cm] {discovers new term} (follower);
            \draw (candidate) edge [transition,loop above,looseness=5,out=110,in=70] node [label,above,text width=2.5cm] {election times out} (candidate);
            \draw (candidate) edge [transition] node [label,above,text width=2.5cm] {receives votes from quorum} (leader);
            \draw (leader) edge [transition,out=215,in=325] node [label,below] {discovers new term} (follower);
        \end{tikzpicture}
    \end{center}%
\end{frame}
...
\begin{frame}
    \label{s:raft1}
    \frametitle{Raft (1): initialisation}
    \footnotesize
    \begin{algorithmic}
        \On{initialisation}
        ...
        \On{node $\mathit{nodeId}$ suspects leader has failed, or on election timeout}
            \State $\mathit{currentTerm} := \mathit{currentTerm} + 1$
            \State $\mathit{currentRole} := \mathsf{candidate};\; \mathit{votedFor} := \mathsf{null}$
            \State $\mathit{votesReceived} := \{\mathit{nodeId}\};\; \mathit{lastTerm} := \mathit{log}[\mathit{log}.\mathsf{length} - 1].\mathsf{term}$
...
\begin{frame}
    \label{s:raft2}
    \frametitle{Raft (2): voting on a new leader}
    \footnotesize
    \begin{algorithmic}
        \State
...
\begin{frame}
    \label{s:raft3}
    \frametitle{Raft (3): collecting votes}
    \footnotesize
    \begin{algorithmic}
        \State
...
\begin{frame}
    \label{s:raft4}
    \frametitle{Raft (4): broadcasting messages}
    \footnotesize
    \begin{algorithmic}
        \State
...
\begin{frame}
    \label{s:raft5}
    \frametitle{Raft (5): replicating from leader to followers}
    \footnotesize
    ...
    \begin{algorithmic}
        \Function{ReplicateLog}{$\mathit{leaderId}, \mathit{followerId}$}
            ...
            \If{$\mathit{sentLength}[\mathit{followerId}] > 0$}
                \State $\mathit{prevLogTerm} := \mathit{log}[\mathit{sentLength}[\mathit{followerId}] - 1].\mathsf{term}$
            \EndIf
            ...
            \State \textbf{send} $(\mathsf{LogRequest}, \mathit{leaderId}, \mathit{currentTerm}, \mathit{sentLength}[\mathit{followerId}],$
            \State \hspace{25pt}$\mathit{prevLogTerm}, \mathit{commitLength}, \mathit{entries})$ to $\mathit{followerId}$
        \EndFunction
        \State
    \end{algorithmic}
\end{frame}
...
\begin{frame}
    \label{s:raft6}
    \frametitle{Raft (6): followers receiving messages}
    \footnotesize
    \makebox[\linewidth][c]{%
        \begin{minipage}{11cm}
            \raggedright
            \begin{algorithmic}
                \On{receiving $(\mathsf{LogRequest}, \mathit{leaderId}, \mathit{term}, \mathit{logLength}, \mathit{logTerm},$\\\hspace{58pt}$\mathit{leaderCommit}, \mathit{entries})$ at node $\mathit{nodeId}$}
                    ...
                    \EndIf
                    \State $\mathit{logOk} := (\mathit{logLength} \le \mathit{log}.\mathsf{length})$
...
\begin{frame}
    \label{s:raft7}
    \frametitle{Raft (7): updating followers' logs}
    \footnotesize
    \begin{algorithmic}
...
\begin{frame}
    \label{s:raft8}
    \frametitle{Raft (8): leader receiving log acknowledgements}
    \footnotesize
    \makebox[\linewidth][c]{%
        \begin{minipage}{12cm}
            \raggedright
...
\begin{frame}
    \label{s:raft9}
    \frametitle{Raft (9): leader committing log entries}
    \footnotesize
...
% Manual leader change, especially for planned maintenance

% The process of choosing a new leader is called \emph{leader election}, and the process of transitioning the leadership to a new node is called \emph{failover}.
