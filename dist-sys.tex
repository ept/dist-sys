...
\vspace{10pt}\noindent Thank you to Jean-Pascal Billaud, Alexandre Fruchaud, and Rishabh Jain for reporting mistakes in an earlier version of these notes.
    ...
    \begin{algorithmic}
        \Function{AppendEntries}{$\mathit{prefixLen}, \mathit{leaderCommit}, \mathit{suffix}$}
            \If{$\mathit{suffix}.\mathsf{length} > 0 \;\wedge\; \mathit{log}.\mathsf{length} > \mathit{prefixLen}$}
                \If{$\mathit{log}[\mathit{prefixLen}].\mathsf{term} \neq \mathit{suffix}[0].\mathsf{term}$}
...
$\mathit{prefixLen}$ is the number of log entries (counted from the beginning of the log) that precede the new $\mathit{suffix}$.
If the follower's log already contains entries at $\mathit{log}[\mathit{prefixLen}]$ and beyond, it compares the term of that existing entry with the term of the first new entry received from the leader.
...
In the case of message duplication, this operation is idempotent, since the new entries are understood to begin at index $\mathit{prefixLen}$ in the log.
...
\begin{frame}
    ...
    \begin{algorithmic}
        \Function{CommitLogEntries}{}
            \While{$\mathit{commitLength} < \mathit{log}.\mathsf{length}$}
                \State $\mathit{acks} := 0$
                \For{each $\mathit{node} \in \mathit{nodes}$}
                    \If{$\mathit{ackedLength}[\mathit{node}] > \mathit{commitLength}$}
                        \State $\mathit{acks} := \mathit{acks} + 1$
                    \EndIf
                \EndFor
                \If{$\mathit{acks} \ge \left\lceil (|\mathit{nodes}| + 1)/2 \right\rceil$}
                    \State deliver $\mathit{log}[\mathit{commitLength}].\mathsf{msg}$ to the application
                    \State $\mathit{commitLength} := \mathit{commitLength} + 1$
                \Else
                    \State \textbf{break}
                \EndIf
            \EndWhile
...
Since $\mathit{commitLength}$ contains the number of log entries that have already been committed and delivered to the application, $\mathit{log}[\mathit{commitLength}]$ is the first log entry that has not yet been committed.
That log entry is ready to commit if it has been acknowledged by a majority quorum of nodes.
Since $\mathit{ackedLength}[\mathit{node}]$ stores the number of log entries acknowledged by $\mathit{node}$, we can simply count the number of nodes for which that number exceeds $\mathit{commitLength}$.
If that number $\mathit{acks}$ constitutes a majority, we deliver $\mathit{log}[\mathit{commitLength}]$ to the application and increment $\mathit{commitLength}$.
...
If $\mathit{acks}$ is less than a majority, we break from the loop and wait for further $\mathsf{LogResponse}$ messages.
...
The logical timestamp for the $\mathsf{set}$ operation is then one plus the maximum timestamp received from the quorum, and for two operations with the same timestamp we break ties based on the ID of the node performing the operation.
(This definition is similar to the Lamport timestamps we saw on \autoref{l:lamport-definition} to \ref{l:lamport-example}.)
...
\begin{frame}
    ...
    \end{itemize}
\end{frame}
\inlineslide{s:collaboration}{\href{https://www.youtube.com/watch?v=OqqliwwG0SM&list=PLeKd45zvjcDFUEv_ohr_HdUFe97RItdiB&index=22}{Start of video section 8.1}\\(\href{https://www.cl.cam.ac.uk/teaching/2021/ConcDisSys/video/16.1_Collaboration.mp4}{mp4 download})}\label{l:collaboration}
...
\supervision{
    ...
    Note that this algorithm uses causal broadcast to ensure that when one updates overwrites another, the causally later (overwriting) update is delivered after the update that it overwrites.
...
\end{frame}
\inlineslide{s:spanner-intro}{\href{https://www.youtube.com/watch?v=oeycOVX70aE&list=PLeKd45zvjcDFUEv_ohr_HdUFe97RItdiB&index=23}{Start of video section 8.2}\\(\href{https://www.cl.cam.ac.uk/teaching/2021/ConcDisSys/video/16.2_Spanner.mp4}{mp4 download})}\label{l:spanner-intro}
